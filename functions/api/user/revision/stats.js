...
export async function onRequestGet(context) {
  ...
  const stats = await env.cgpsc_quiz_db.prepare(`
    SELECT
      ...
      SUM(CASE WHEN wq.interval_days >= 30
               THEN 1 ELSE 0 END)                      AS mastered,
      SUM(CASE WHEN wq.interval_days < 7
               THEN 1 ELSE 0 END)                      AS learning,
      SUM(wq.wrong_count)                               AS total_wrongs,
      AVG(wq.ease_factor)                               AS avg_ease,
      MAX(wq.wrong_count)                               AS hardest_wrong_count
  ...
  const { results: activity } = await env.cgpsc_quiz_db.prepare(`
    SELECT
      date(wq.last_wrong_at) AS day,
      COUNT(*)            AS revised_count
    FROM wrong_questions wq
    JOIN questions q ON wq.question_id = q.id
    WHERE wq.user_id = ?
      AND wq.last_wrong_at >= datetime('now', '-14 days')
  ...
  const { results: pendingByInterval } = await env.cgpsc_quiz_db.prepare(`
    ...
    GROUP BY wq.interval_days
    ORDER BY wq.interval_days ASC
  `).bind(user.id).all();
