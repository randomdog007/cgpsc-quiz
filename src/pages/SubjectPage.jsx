...
export default function SubjectPage(props) {
  const { 
    ...
    headerProps, lang, profile, history, supabase
  } = props;
  
  const [quizCounts, setQuizCounts] = React.useState({});

  React.useEffect(() => {
    if (!topics || topics.length === 0 || !supabase) return;
    
    // Always fetch counts directly from quizzes table
    // (Nested select sometimes returns empty arrays due to RLS/limits)

    const fetchCounts = async () => {
      try {
        const res = await fetch(`/api/catalog/topic-quiz-counts?subjectId=${selectedSubject.id}`);
        ...
        // Ensure all topics have at least a 0 count mapped
        topics.forEach(t => {
          if (counts[t.id] === undefined) counts[t.id] = 0;
        });
        setQuizCounts(counts);
      } catch (e) {
        console.error("Error fetching quiz counts:", e);
        const counts = {};
              ...
              {topics.map((topic, idx) => {
                ...
                return (
                  <div key={topic.id} onClick={() => onOpenTopic(topic)} style={{ 
                    ...
                    {/* Quiz count pill + arrow — visible on all screen sizes */}
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0, marginLeft: 10 }}>
                      {accAvg > 0 ? (
                        ...
                        <div style={{ background: "rgba(46, 122, 82, 0.1)", color: "var(--teal)", padding: "5px 10px", borderRadius: 20, fontSize: 12, fontWeight: 700, whiteSpace: "nowrap" }}>
                          {quizCounts[topic.id] === undefined ? "Loading…" : (quizCounts[topic.id] === 0 ? "No quizzes" : quizCounts[topic.id] === 1 ? "1 Quiz" : `${quizCounts[topic.id]} Quizzes`)}
                        </div>
                      )}
